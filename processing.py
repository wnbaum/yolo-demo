...
darknet = tf.keras.models.load_model('models/yolo_weights.h5')
...
def draw_boxes(image_, boxes, labels, verbose = True):
    ...
    font = ImageFont.truetype(font='fonts/LiberationMono-Regular.ttf',
...
def detect_image(image_pil, obj_thresh = 0.4, nms_thresh = 0.45, darknet=darknet, net_h=416, net_w=416, anchors=anchors, labels=labels):
    ...
    image_shape = np.array(image_pil).shape
